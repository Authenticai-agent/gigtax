 ...
 */
/**
 * SUPERSEDED — not called by any page.
 *
 * These are the intermediate groupings from collapsing the 231 pair pages:
 * first by credit case, then by no-tax work state, then by no-tax home state.
 * The per-state merge that followed made all of them unreachable — the section
 * is now one page per taxing state, generated by homeCommutePages().
 *
 * Left in place rather than removed: ground rule 5 says nothing gets deleted
 * without being asked. They are dead, they generate the old URL shapes, and
 * they should go — say the word.
 */
