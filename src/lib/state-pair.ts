...
export function pairOutcome(homeCode: string, workCode: string, income = 70000, status = 'single'): PairOutcome {
  ...
  const exemptionForm = reciprocal ? reciprocity[homeCode]?.exemptionForm ?? null : null;
...
function buildHeadline(type: ArrangementType, home: StateData, work: StateData): string {
  switch (type) {
    ...
    case 'az-wec':
      return `${work.name} still taxes your wages, but as a ${home.name} resident you can skip Arizona withholding (Form WEC) and settle up with a credit.`;
    case 'credit':
      return `${home.name} and ${work.name} have no reciprocity agreement, so you file a ${work.name} nonresident return and ${home.name} credits the tax you paid.`;
    case 'single-state':
      return noTax(work)
...
function buildExplanation(type: ArrangementType, home: StateData, work: StateData, income: number, workTaxFull: number, homeTaxFull: number, form: string | null): string {
  ...
  switch (type) {
    case 'reciprocity':
      return `Because of the reciprocity agreement, ${work.name} will not tax the wages you earn there. File ${form ?? 'the nonresident exemption form'} with your employer to stop ${work.name} withholding, and report all ${inc} on your ${home.name} resident return. You never file a ${work.name} return for these wages.`;
    case 'az-wec':
      return `Arizona is not a reciprocity state. As a ${home.name} resident you may file Arizona Form WEC to skip Arizona withholding, but Arizona income tax still applies to the ${inc} you earn there — you file an Arizona nonresident return (Form 140NR) and claim a credit for taxes paid so you are not taxed twice. Estimated Arizona tax ${formatMoney(workTaxFull)}; ${home.name} tax ${formatMoney(homeTaxFull)}.`;
    case 'credit':
